...
class Movies(db.Model):
    ...
    reviews = db.relationship(
        ...
        primaryjoin="Movies.moviename==Reviews.moviename"
    )
...
class Reviews(db.Model):
    ...
    moviename = db.Column(db.String(100),db.ForeignKey('movieslist.moviename', ondelete='CASCADE'),  nullable=False)
...
@app.route('/all_movies/<int:No>', methods=['GET'])
def allMovies(No):
    if No < 1:
    ...
    pagination = Movies.query.paginate(page=No, per_page=12, error_out=False)
    allMovie = pagination.items
    return jsonify([movie.to_dict() for movie in allMovie]) if allMovie else jsonify({"Message": "No movies available"})
